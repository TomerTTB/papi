...
test.describe('Headers Validation', () => {
    ...
    for (const endpoint of endpointsToTest) {
        ...
        // Special endpoint-specific test block for specific header validation tests
        switch(endpoint.path) {
            case '@/websdk/init':
                // WebSDK Init - Specific implementation of header validation tests
                
                // Verifies that the server returns 401 when specific required headers are missing
                // Tests: Glasseson-Client-Id, Glasseson-Profile-Name, Referer
                test(getTestTitle('Missing Headers Validation', endpoint), async ({ request }) => {
                    ...
                    for (const missingHeader of requiredHeaders) {
                        ...
                        if (missingHeader === 'Glasseson-Client-Id') {
                            // WebSDK specific behavior for missing Client ID
                            expect(errorResponse.error_message).toContain('Client id does not exist. You are connected to');
                            expect(errorResponse.error_code).toBe(401);
                        } else if (missingHeader === 'Glasseson-Profile-Name') {
                            // WebSDK specific behavior for missing Profile Name
                            expect(errorResponse).toBe('Something went wrong');
                        } else if (missingHeader === 'Referer') {
                            // WebSDK specific behavior for missing Referer
        ...
        //  Tests: Glasseson-Client-Id, Glasseson-Profile-Name, Referer
                test(getTestTitle('All Required Headers Missing', endpoint), async ({ request }) => {
                    ...
                    // Verify status code - WebSDK specific behavior
                    expect(response.status(), 'Server should return 401 when all required headers are missing').toBe(401);
                
                    // Parse response JSON - WebSDK specific validation
                    const errorResponse = await response.json();
                
                    // Verify expected error message for WebSDK
                    expect(errorResponse.error_message).toContain('Client id does not exist');
                });
                
                
                /**
                 * Test Empty Header Values - WebSDK implementation
                ...
                test(getTestTitle('Empty Headers Validation', endpoint), async ({ request }) => {
                    ...
                    for (const emptyHeader of requiredHeaders) {
                        ...
                        // Verify status code - WebSDK specific validation
                        expect(response.status(), 
                            `Server should return 401 when ${emptyHeader} is empty`
                        ).toBe(401);
                
                        // Different error responses based on the empty header for WebSDK
                        const errorResponse = await response.json();
                
                        if (emptyHeader === 'Glasseson-Client-Id') {
                            expect(errorResponse.error_message).toContain('Client id does not exist');
                        } else if (emptyHeader === 'Glasseson-Profile-Name') {
                            expect(errorResponse.error_message).toContain('Profile name does not exist for the provided client id');
                        } else if (emptyHeader === 'Referer') {
                ...
                //  Test to verify that the server returns 401 when all required headers are empty - WebSDK implementation
                ...
                /**
                 * Test Invalid Header Values - WebSDK implementation
                ...
                test(getTestTitle('Invalid Header Values', endpoint), async ({ request }) => {
                    ...
                    for (const invalidHeader of requiredHeaders) {
                        ...
                        // Different error messages based on invalid header - WebSDK specific
                        if (invalidHeader === 'Glasseson-Client-Id') {
                            expect(errorResponse.error_message).toContain('Client id does not exist');
                        } else if (invalidHeader === 'Glasseson-Profile-Name') {
                            expect(errorResponse.error_message).toContain('Profile name does not exist for the provided client id');
                        } else if (invalidHeader === 'Referer') {
                            expect(errorResponse).toEqual({
                                error_code: 401,
                                error_message: 'The provided client id does not match with the provided domain. Contact support@6over6.com if needed'
                            });
                        }
                    }
                });
                ...
                //  Test to verify that the server returns 401 when all required headers are invalid - WebSDK implementation
                test(getTestTitle('All Invalid Header Values', endpoint), async ({ request }) => {
                    ...
                    // Check the response status code - WebSDK specific validation
                    expect(response.status(),
                    ...
                    // Ensure the error message contains specific strings related to invalid headers - WebSDK specific
                    expect(errorResponse.error_message).toContain('Client id does not exist');
                });
                break;
            
            case '@/mobile/init':
                // Mobile Init - Specific implementation of header validation tests
                ...
                test(getTestTitle('Missing Headers Validation', endpoint), async ({ request }) => {
                    ...
                    for (const missingHeader of requiredHeaders) {
                        ...
                        if (missingHeader === 'Glasseson-Client-Id') {
                            // Mobile specific behavior for missing Client ID
                            expect(errorResponse.error_message).toContain('Client id does not exist. You are connected to');
                            expect(errorResponse.error_code).toBe(401);
                        } else if (missingHeader === 'Glasseson-Profile-Name') {
                            // Mobile specific behavior for missing Profile Name
                            expect(errorResponse).toBe('Something went wrong');
                        } else if (missingHeader === 'Glasseson-Native-App-Id') {
                            // Mobile specific behavior for missing Native App ID
                            expect(errorResponse).toEqual({
                                error_code: 401,
                                error_message: 'No Domain was provided for client Id. Contact support@6over6.com if needed'
                            });
                ...
                test(getTestTitle('All Required Headers Missing', endpoint), async ({ request }) => {
                    ...
                    // Verify status code - Mobile specific behavior
                    expect(response.status(), 'Server should return 401 when all required headers are missing').toBe(401);
                
                    // Parse response JSON - Mobile specific validation
                    const errorResponse = await response.json();
                
                    // Verify expected error message for Mobile
                    expect(errorResponse.error_message).toContain('Client id does not exist');
                });
                
                
                /**
                 * Test Empty Header Values - Mobile implementation
                 * Verifies that the server returns 401 when required headers are present but empty
                 * Tests: Glasseson-Client-Id, Glasseson-Profile-Name, Glasseson-Native-App-Id
                 */
                test(getTestTitle('Empty Headers Validation', endpoint), async ({ request }) => {
                    ...
                    for (const emptyHeader of requiredHeaders) {
                        ...
                        // Verify status code - Mobile specific validation
                        expect(response.status(), 
                            `Server should return 401 when ${emptyHeader} is empty`
                        ).toBe(401);
                
                        // Different error responses based on the empty header for Mobile
                        const errorResponse = await response.json();
                        ...
                        if (emptyHeader === 'Glasseson-Client-Id') {
                            ...
                            expect(errorResponse.error_message).toContain('No Domain was provided for client Id. Contact support@6over6.com if needed');
                        }
                    }
                });
             
                
                //  Test to verify that the server returns 401 when all required headers are empty - Mobile implementation
                test(getTestTitle('All Required Headers Empty', endpoint), async ({ request }) => {
                    // Clone headers to avoid modifying the original config
                    const headers = { ...endpoint.config.requiredHeaders };
                ...
                /**
                 * Test Invalid Header Values - Mobile implementation
                 * Verifies that the server returns 401 when required headers have invalid values
                 */
                test(getTestTitle('Invalid Header Values', endpoint), async ({ request }) => {
                    const requiredHeaders = [
                    ...
                    for (const invalidHeader of requiredHeaders) {
                        ...
                        // Different error messages based on invalid header - Mobile specific
                        if (invalidHeader === 'Glasseson-Client-Id') {
                            expect(errorResponse.error_message).toContain('Client id does not exist');
                        } else if (invalidHeader === 'Glasseson-Profile-Name') {
                            expect(errorResponse.error_message).toContain('Profile name does not exist for the provided client id');
                            ...
                            expect(errorResponse).toEqual({
                                error_code: 401,
                                error_message: 'The provided client id does not match with the provided app id. Contact support@6over6.com if needed'
                            });
                        }
                    }
                });
                ...
                //  Test to verify that the server returns 401 when all required headers are invalid - Mobile implementation
                test(getTestTitle('All Invalid Header Values', endpoint), async ({ request }) => {
                    const requiredHeaders = [
                        'Glasseson-Client-Id',
                        'Glasseson-Profile-Name',
                    ...
                    // Check the response status code - Mobile specific validation
                    expect(response.status(),
                        'Server should return 401 when all headers are invalid'
                    ).toBe(401);
                    ...
                    // Ensure the error message contains specific strings related to invalid headers - Mobile specific
                    expect(errorResponse.error_message).toContain('Client id does not exist');
                });
                break;

            case '@/pd/verify':
                // PD Verify - Specific implementation of header validation tests
        ...
        test(getTestTitle('Missing Headers Validation', endpoint), async ({ request }) => {
            ...
            for (const missingHeader of requiredHeaders) {
                ...
                if (missingHeader === 'Glasseson-Client-Id') {
                            // PD specific behavior for missing Client ID
                    expect(errorResponse.error_message).toContain('Client id does not exist. You are connected to');
                    expect(errorResponse.error_code).toBe(401);
                } else if (missingHeader === 'Glasseson-Profile-Name') {
                            // PD specific behavior for missing Profile Name
                    expect(errorResponse).toBe('Something went wrong');
                } else if (missingHeader === 'Referer') {
                            // PD specific behavior for missing Referer
                    expect(errorResponse).toEqual({
                        error_code: 401,
                        error_message: 'No Domain was provided for client Id. Contact support@6over6.com if needed'
                    });
        ...
        test(getTestTitle('All Required Headers Missing', endpoint), async ({ request }) => {
            ...
            const response = await executeRequest(test, request, endpoint, headers);
                    ...
                    // Verify status code - PD specific behavior
            expect(response.status(), 'Server should return 401 when all required headers are missing').toBe(401);
        
                    // Parse response JSON - PD specific validation
            const errorResponse = await response.json();
        
                    // Verify expected error message for PD
            expect(errorResponse.error_message).toContain('Client id does not exist');
        });
        
        
        /**
                 * Test Empty Header Values - PD implementation
         * Verifies that the server returns 401 when required headers are present but empty
         * Tests: Glasseson-Client-Id, Glasseson-Profile-Name, Referer
         */
        test(getTestTitle('Empty Headers Validation', endpoint), async ({ request }) => {
            ...
            for (const emptyHeader of requiredHeaders) {
                ...
                const response = await executeRequest(test, request, endpoint, headers);
                        ...
                        // Verify status code - PD specific validation
                expect(response.status(), 
                    `Server should return 401 when ${emptyHeader} is empty`
                ).toBe(401);
        
                        // Different error responses based on the empty header for PD
                const errorResponse = await response.json();
                ...
                //  Test to verify that the server returns 401 when all required headers are empty - PD implementation
        test(getTestTitle('All Required Headers Empty', endpoint), async ({ request }) => {
            // Clone headers to avoid modifying the original config
            const headers = { ...endpoint.config.requiredHeaders };
        ...
        /**
                 * Test Invalid Header Values - PD implementation
         * Verifies that the server returns 401 when required headers have invalid values
         */
        test(getTestTitle('Invalid Header Values', endpoint), async ({ request }) => {
            const requiredHeaders = [
            ...
            for (const invalidHeader of requiredHeaders) {
                ...
                const errorResponse = await response.json();
                        ...
                        // Different error messages based on invalid header - PD specific
                if (invalidHeader === 'Glasseson-Client-Id') {
                    expect(errorResponse.error_message).toContain('Client id does not exist');
                } else if (invalidHeader === 'Glasseson-Profile-Name') {
                    expect(errorResponse.error_message).toContain('Profile name does not exist for the provided client id');
                ...
                //  Test to verify that the server returns 401 when all required headers are invalid - PD implementation
        test(getTestTitle('All Invalid Header Values', endpoint), async ({ request }) => {
            const requiredHeaders = [
                'Glasseson-Client-Id',
                'Glasseson-Profile-Name',
            ...
            const response = await executeRequest(test, request, endpoint, headers);
                    ...
                    // Check the response status code - PD specific validation
            expect(response.status(),
                'Server should return 401 when all headers are invalid'
            ).toBe(401);
            ...
            const errorResponse = await response.json();
                    ...
                    // Ensure the error message contains specific strings related to invalid headers - PD specific
            expect(errorResponse.error_message).toContain('Client id does not exist');
        });
                break;
        }
