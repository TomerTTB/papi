...
test.describe('Headers Validation', () => {
    ...
    for (const endpoint of endpointsToTest) {
        ...
        test(getTestTitle('Required AWS Headers Present', endpoint), async ({ request }) => {
            ...
            // List of required headers for Attachments API
        ...
        //  Verifies that the server returns 401/403 when specific required headers are missing
        //  Tests: Glasseson-Client-Id, Glasseson-Profile-Name, Glasseson-Native-App-Id, X-Api-Key
        ...
        test(getTestTitle('Missing Headers Validation', endpoint), async ({ request }) => {
            ...
            for (const missingHeader of requiredHeaders) {
                ...
                if (missingHeader === 'X-Api-Key') {
                    // Special case for X-Api-Key
                    expect(response.status()).toBe(403);
                    const errorResponse = await response.json();
                    expect(errorResponse).toEqual({ message: "Forbidden" });
                } else {
                    // Other headers
                    ...
                    if (missingHeader === 'Glasseson-Profile-Name') {
                        expect(errorResponse).toContain("No profile name: 'None' for client Id:");
                    } else if (missingHeader === 'Glasseson-Native-App-Id') {
                        expect(errorResponse).toContain("Native app id: 'None' not allowed for client Id:");
                    } else if (missingHeader === 'Glasseson-Client-Id') {
                        expect(errorResponse).toContain("Wrong client Id: 'None'");
                    }
                }
            }
        });
         ...
         * Verifies that the server returns 401/403 when required headers are present but empty
         * Tests: Glasseson-Client-Id, Glasseson-Profile-Name, Glasseson-Native-App-Id, X-Api-Key
         * Different error messages for different empty headers:
         * - Glasseson-Client-Id: "Wrong client Id: ''"
         * - Glasseson-Profile-Name: "No profile name: '' for client Id:"
         * - Glasseson-Native-App-Id: "Native app id: '' not allowed for client Id:"
         * - X-Api-Key: { message: "Forbidden" }
         */
        test(getTestTitle('Empty Headers Validation', endpoint), async ({ request }) => {
            ...
            for (const emptyHeader of requiredHeaders) {
                ...
                if (emptyHeader === 'X-Api-Key') {
                    // Special case for X-Api-Key
                    expect(response.status(), 
                        `Server should return 403 when ${emptyHeader} is empty`
                    ).toBe(403);
                    const errorResponse = await response.json();
                    expect(errorResponse).toEqual({ message: "Forbidden" });
                } else {
                    ...
                    if (emptyHeader === 'Glasseson-Profile-Name') {
                        expect(errorResponse).toContain("No profile name: '' for client Id:");
                    } else if (emptyHeader === 'Glasseson-Native-App-Id') {
                        expect(errorResponse).toContain("Native app id: '' not allowed for client Id:");
                    } else if (emptyHeader === 'Glasseson-Client-Id') {
                        expect(errorResponse).toContain("Wrong client Id: ''");
                    }
                }
            }
        });
         ...
         * Verifies that the server returns 401/403 when required headers have invalid values
         * Tests: Glasseson-Client-Id, Glasseson-Profile-Name, Glasseson-Native-App-Id, X-Api-Key
         * Different error messages for different invalid headers:
         * - Glasseson-Client-Id: "Wrong client Id: 'invalid_value'"
         * - Glasseson-Profile-Name: "No profile name: 'invalid_value' for client Id:"
         * - Glasseson-Native-App-Id: "Native app id: 'invalid_value' not allowed for client Id:"
         * - X-Api-Key: { message: "Forbidden" }
         */
        test(getTestTitle('Invalid Header Values', endpoint), async ({ request }) => {
            ...
            for (const invalidHeader of requiredHeaders) {
                ...
                if (invalidHeader === 'X-Api-Key') {
                    // Special case for X-Api-Key
                    expect(response.status(), 
                        `Server should return 403 when ${invalidHeader} is invalid`
                    ).toBe(403);
                    const errorResponse = await response.json();
                    expect(errorResponse).toEqual({ message: "Forbidden" });
                } else {
                    // Other headers
                    expect(response.status(), 
                        `Server should return 401 when ${invalidHeader} is invalid`
                    ).toBe(401);
        
                    const errorResponse = await response.json();
        
                    if (invalidHeader === 'Glasseson-Profile-Name') {
                        expect(errorResponse).toContain("No profile name: 'invalid_value' for client Id:");
                    } else if (invalidHeader === 'Glasseson-Native-App-Id') {
                        expect(errorResponse).toContain("Native app id: 'invalid_value' not allowed for client Id:");
                    } else if (invalidHeader === 'Glasseson-Client-Id') {
                        expect(errorResponse).toContain("Wrong client Id: 'invalid_value'");
                    }
                }
            }
        });
